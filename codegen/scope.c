...
void scope_enter(int function) {
	//function determines if we reset the local variable count
	...
	if(function){
		head->scope_variables = 0;
	} else {//set the scope_variables to the next scope variables
		head->scope_variables = head->next->scope_variables;
	}
}
